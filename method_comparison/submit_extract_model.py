...
for unit in units:
    ...
    for layer in layers:
        ...
        for sparsity in sparsities:
            call_str = 'python extract_model.py --method %s --unit %s --layer %s --sparsity %s --config %s --data-path %s --device %s'%(method,str(unit),layer,str(sparsity),config,data_path,device)
